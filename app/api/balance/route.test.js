...
it('uses the canonical Project Pass repository instead of the removed entitlement columns', () => {
  ...
  expect(source).toContain('db = createAdminClient()')
  expect(source).toContain("lookupActiveProjectPassForProduct(db, {")
  expect(source).toContain("productId: `katedra_pass_${lockResult.lock.productKey}`")
  expect(source).toContain("lookupActiveProjectPass(db, { userId: user.id, projectId: project.projectId })")
  expect(source).toContain("resolveOwnedProjectResult(db, { userId: user.id, projectId })")
