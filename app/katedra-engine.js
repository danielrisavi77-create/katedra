...
// meta je opcionalan i nosi DVA nezavisna sloja podataka koja mogu koegzistirati
// na istom zapisu: {kind} = strojno čitljiva taksonomija DOGAĐAJA (v.
// LOG_KIND_LABELS — milestone/ai_declaration/prompt_generated/session_started/
// ai_response/lekta_check/lekta_fix_round, + specifična polja po vrsti poput
// phaseId/level/mode/files/lekta_result/done/skip/open), i {aiGenerated, stage,
// tool, model, reviewed} = AI-provenance signal (je li ZAPIS AI sadržaj, koji
// model, je li student stvarno pregledao) — potonje ima smisla prvenstveno na
// kind:'ai_response' zapisima. Stariji zapisi (bez meta) ostaju čitljivi —
// renderiraju se bez dodatnih stupaca, ne bacaju grešku.
...
async function liveBegin(promptText){
  ...
  rpLog('Pisanje u aplikaciji pokrenuto' + (chat.files.length ? ' uz ' + chat.files.length + ' prilog' + (chat.files.length===1?'':'a') : ''),
    {kind:'session_started', files: chat.files.map(f => f.name), aiGenerated:false, stage:'live'});
