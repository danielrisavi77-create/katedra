...
const configured = {
  ...
  KATEDRA_VERIFIER_PROVIDER_URL: 'https://verifier.katedra.example/run',
  KATEDRA_VERIFIER_PROVIDER_KEY: 'configured-verifier-key',
  KATEDRA_VERIFIER_PROVIDER_MODEL: 'configured-verifier-model',
...
const evidence = {
  canonicalContract: {
    ...
    environment: 'staging',
    reference: 'lekta-migration-0085/sql-audit-2026-09-20',
  ...
  dependencyAudit: {
    verified: true,
    verifiedAt: '2026-09-20T10:10:00.000Z',
    environment: 'release',
    reference: 'npm-audit-report-2026-09-20',
    commitSha: 'a'.repeat(40),
...
describe('evaluateReleaseReadiness', () => {
  ...
  it('rejects evidence URLs that embed credentials', () => {
    const result = evaluateReleaseReadiness(configured, {
      ...evidence,
      canonicalContract: {
        ...evidence.canonicalContract,
        deploymentUrl: 'https://operator:secret@staging.katedra.example',
      },
    })

    expect(result.ok).toBe(false)
    expect(result.external).toContain('canonical_contract_evidence')
  })
})
