...
it('keeps the gate optional and requires its URL/token only on explicit opt-in', () => {
  expect(evaluateProductionEnvironment({ ...configured, KATEDRA_GATE_VERIFIER_REQUIRED: 'false' })).toEqual({ ok: true, missing: [], invalid: [] })
  expect(evaluateProductionEnvironment({ ...configured, KATEDRA_GATE_VERIFIER_REQUIRED: 'true' })).toEqual({ ok: false, missing: ['KATEDRA_GATE_VERIFIER_URL', 'KATEDRA_GATE_VERIFIER_TOKEN'], invalid: [] })
  const ready = { ...configured, KATEDRA_GATE_VERIFIER_REQUIRED: 'true', KATEDRA_GATE_VERIFIER_URL: 'https://gate.example', KATEDRA_GATE_VERIFIER_TOKEN: 'fixture-token', KATEDRA_GATE_VERIFIER_TIMEOUT_MS: '120000' }
  expect(evaluateProductionEnvironment(ready).ok).toBe(true)
  expect(evaluateProductionEnvironment({ ...ready, KATEDRA_GATE_VERIFIER_URL: 'http://gate.example', KATEDRA_GATE_VERIFIER_TIMEOUT_MS: '-1' }).invalid).toEqual(['KATEDRA_GATE_VERIFIER_URL', 'KATEDRA_GATE_VERIFIER_TIMEOUT_MS'])
})
