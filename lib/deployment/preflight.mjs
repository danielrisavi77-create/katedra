...
// Optional until the gate service has been deployed and explicitly made required.
export const OPTIONAL_GATE_VERIFIER_ENV = [
  'KATEDRA_GATE_VERIFIER_URL',
  'KATEDRA_GATE_VERIFIER_TOKEN',
  'KATEDRA_GATE_VERIFIER_REQUIRED',
  'KATEDRA_GATE_VERIFIER_TIMEOUT_MS',
]
...
export function evaluateProductionEnvironment(env = process.env) {
  ...
  if (env.KATEDRA_GATE_VERIFIER_REQUIRED === 'true') {
    for (const key of ['KATEDRA_GATE_VERIFIER_URL', 'KATEDRA_GATE_VERIFIER_TOKEN']) {
      if (!isConfigured(env[key])) missing.push(key)
    }
  }
  if (isConfigured(env.KATEDRA_GATE_VERIFIER_URL)) {
    try {
      if (new URL(env.KATEDRA_GATE_VERIFIER_URL).protocol !== 'https:') invalid.push('KATEDRA_GATE_VERIFIER_URL')
    } catch { invalid.push('KATEDRA_GATE_VERIFIER_URL') }
  }
  if (env.KATEDRA_GATE_VERIFIER_REQUIRED && !['true', 'false'].includes(env.KATEDRA_GATE_VERIFIER_REQUIRED)) invalid.push('KATEDRA_GATE_VERIFIER_REQUIRED')
  if (env.KATEDRA_GATE_VERIFIER_TIMEOUT_MS && (!Number.isFinite(Number(env.KATEDRA_GATE_VERIFIER_TIMEOUT_MS)) || Number(env.KATEDRA_GATE_VERIFIER_TIMEOUT_MS) <= 0)) invalid.push('KATEDRA_GATE_VERIFIER_TIMEOUT_MS')
