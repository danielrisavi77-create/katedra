...
import { isAgenticWorkspaceAvailable } from './agentic-availability'
...
configured.KATEDRA_MATERIALS_ENABLED = 'true'
configured.KATEDRA_MATERIAL_DELETE_RPC_CONTRACT = 'v1'
configured.ANTHROPIC_API_KEY = 'test-provider-key'
...
describe('evaluateAgenticStagingEnvironment', () => {
  it.each(REQUIRED_AGENTIC_STAGING_ENV)('agrees with runtime availability when %s is absent', (key) => {
    const environment = { ...configured, [key]: undefined }
    expect(evaluateAgenticStagingEnvironment(environment).ok).toBe(false)
    expect(isAgenticWorkspaceAvailable(environment)).toBe(false)
  })
  ...
  it.each([
    ['missing', 'KATEDRA_MATERIALS_ENABLED', undefined, 'missing'],
    ['invalid', 'KATEDRA_MATERIALS_ENABLED', 'false', 'invalid'],
    ['missing', 'KATEDRA_MATERIAL_DELETE_RPC_CONTRACT', undefined, 'missing'],
    ['invalid', 'KATEDRA_MATERIAL_DELETE_RPC_CONTRACT', 'v2', 'invalid'],
    ['missing', 'ANTHROPIC_API_KEY', undefined, 'missing'],
  ...
  it('requires the canonical material deletion contract when material uploads are enabled', () => {
    const result = evaluateAgenticStagingEnvironment({
      ...
      KATEDRA_MATERIAL_DELETE_RPC_CONTRACT: undefined,
    })
