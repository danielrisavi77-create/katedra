...
export function evaluateAgenticStagingEnvironment(env = process.env) {
  ...
  if (isConfigured(env.KATEDRA_MATERIAL_DELETE_RPC_CONTRACT) && env.KATEDRA_MATERIAL_DELETE_RPC_CONTRACT !== 'v1') {
    invalid.push('KATEDRA_MATERIAL_DELETE_RPC_CONTRACT')
  }
  if (isConfigured(env.KATEDRA_MATERIALS_ENABLED) && env.KATEDRA_MATERIALS_ENABLED !== 'true') {
    invalid.push('KATEDRA_MATERIALS_ENABLED')
  }
