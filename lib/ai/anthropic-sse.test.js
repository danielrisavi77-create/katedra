...
describe('createAnthropicUsageParser', () => {
  ...
  it('flushes a UTF-8 code point split across network chunks', () => {
    const parser = createAnthropicUsageParser()
    const encoded = bytes('data: {"type":"message_start","message":{"usage":{"input_tokens":12},"model":"č"}}\n')
    const marker = new TextEncoder().encode('č')
    const split = Array.from(encoded).indexOf(marker[0]) + 1

    parser.push(encoded.slice(0, split))
    parser.push(encoded.slice(split))
    parser.finish()

    expect(parser.usage()).toEqual({ inputTokens: 12, outputTokens: 0 })
  })
})
