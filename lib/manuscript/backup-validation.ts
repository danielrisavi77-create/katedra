...
function nonEmptyString(value: unknown, maxLength = MAX_FIELD_LENGTH): value is string {
  return validString(value, maxLength) && value.trim().length > 0
}
...
function validateSection(value: unknown): value is ManuscriptSectionV1 {
  if (!isRecord(value)) return false
  if (!nonEmptyString(value.id, 200) || !validString(value.title, MAX_SECTION_TITLE_LENGTH)) return false
...
function validateSource(value: unknown): boolean {
  if (!isRecord(value) || !nonEmptyString(value.id, 200) || !validString(value.title, MAX_SECTION_TITLE_LENGTH) || typeof value.verified !== 'boolean') return false
...
export function validateManuscriptBackup(value: unknown, projectId: string): BackupResult {
  ...
  const sections = value.sections as Array<{ id: string }>
  if (new Set(sections.map((section) => section.id)).size !== sections.length) return failure('invalid', 'Backup ima duplicirane ID-jeve sekcija.')
  if (!validString(value.activeSectionId, 200) || !sections.some((section) => section.id === value.activeSectionId)) return failure('invalid', 'Aktivna sekcija nije valjana.')
  ...
  const sources = value.sources as Array<{ id: string }>
  if (new Set(sources.map((source) => source.id)).size !== sources.length) return failure('invalid', 'Backup ima duplicirane ID-jeve izvora.')
