...
describe('project lock', () => {
  ...
  it('writes the paid lock through the canonical Lekta RPC', async () => {
    const rpc = vi.fn().mockResolvedValue({ data: { ...baseInput, status: 'locked' }, error: null })
    const db = { rpc }
  ...
  it('accepts the canonical RPC lock id response and preserves the immutable snapshot', async () => {
    const db = { rpc: vi.fn().mockResolvedValue({ data: { lock_id: 'lock-1' }, error: null }) }
    await expect(lockPaidProject(db, baseInput)).resolves.toMatchObject({
      ok: true,
      lock: { projectId: baseInput.projectId, topic: baseInput.topic, paymentId: baseInput.paymentId, status: 'locked' },
      lockId: 'lock-1',
    })
  })
    ...
    const db = {
      from(table: string) {
        ...
        const query = {
          ...
          eq() { return query },
          async maybeSingle() {
            return { data: { ...baseInput, status: 'locked' }, error: null }
          },
        }
        return query
      },
