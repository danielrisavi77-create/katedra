...
describe('agent result payload storage', () => {
  it('stores the generated result privately and registers only its manifest', async () => {
    ...
    const stored = await storeAgentStepResult({ rpc, storage: { from: vi.fn(() => ({ upload, remove })) } }, {
    ...
    expect(rpc).toHaveBeenCalledWith('register_agent_payload', expect.objectContaining({
      p_run_id: 'run-1',
      p_material_id: expect.stringMatching(/^agent-result:/),
      p_storage_path: expect.stringContaining('/results/'),
    }))
  })
  ...
  it('loads only result payloads belonging to the requested run and project', async () => {
    const manifests = {
      list: vi.fn(async () => [
        ...
        { materialId: 'agent-result:other', projectId: 'project-2', runId: 'run-2', storageBucket: 'bucket', storagePath: 'other.json', manifestPath: 'other.manifest.json' },
      ]),
    }
    ...
    const storage = { download: vi.fn(async (path: string) => path === 'manifest.json'
      ? JSON.stringify({ ...payload, storagePath: 'result.json', materialId: 'agent-result:step-1:1' })
      : JSON.stringify(payload)) }

    const loaded = await loadAgentRunResults(manifests, storage, { runId: 'run-1', projectId: 'project-1' })
    expect(loaded).toHaveLength(1)
    expect(loaded[0]).toMatchObject({ stepId: 'step-1', output: 'Novi odlomak rada.', projectId: 'project-1' })
