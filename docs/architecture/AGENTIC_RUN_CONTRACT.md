...
- Novi run prvo ima status `initializing`; worker ga ne smije claimati dok
  `run-context` manifest nije registriran i dok Katedra ne pozove
  `activate_agent_run`.
- Run context se ne smije mijenjati dok je run u statusu `running`; promjena
