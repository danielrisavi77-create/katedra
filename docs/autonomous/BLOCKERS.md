...
An independent verifier gateway is now required for agentic activation and
remains fail-closed. It requires
`KATEDRA_VERIFIER_PROVIDER_URL`, `KATEDRA_VERIFIER_PROVIDER_KEY`,
`KATEDRA_VERIFIER_PROVIDER_MODEL` and
`KATEDRA_VERIFIER_POLICY_APPROVED=true`. It independently annotates exact
claim-to-passage matches; the worker blocks a result when those passages have
not been verified. This routing is not enabled by default and still needs a
staging proof that the second provider returns the bounded decision contract,
usage data and failure semantics.
