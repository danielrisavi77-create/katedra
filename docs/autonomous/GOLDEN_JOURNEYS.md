...
| G2 | Guest registration continues the same project without duplication | BLOCKED_EXTERNAL | Local auth redirect tests prove explicit projectId preservation and safe fallback; authenticated Supabase staging credentials and real attach flow remain external |
| G3 | Registered Free user sees roadmap, free capabilities and paid gates | BLOCKED_EXTERNAL | Local capability matrix proves bounded `contextual_ai` and unknown chat capabilities fail closed; the authenticated Free account and provider path requires staging auth/provider |
...
| G9 | Network, session, provider, balance, checkout and malformed-state failures recover honestly | BLOCKED_EXTERNAL | Local route/unit failure tests and Playwright malformed `rp_manifest`/`rp_state` recovery pass; authenticated provider, session and payment failure recovery requires staging |
