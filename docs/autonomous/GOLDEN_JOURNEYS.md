...
| G9 | Network, session, provider, balance, checkout and malformed-state failures recover honestly | BLOCKED_EXTERNAL | Local route/unit failure tests, Playwright malformed-state recovery, active-run resume, stale-run marker recovery, terminal failed-run recovery, local proposal-decision recovery, worker project-lock fail-closed coverage and lockfile-aligned browser workflow checks pass; authenticated provider, session and payment failure recovery requires staging |
