...
| G3 | Registered Free user sees roadmap, free capabilities and paid gates | BLOCKED_EXTERNAL | Local capability matrix proves bounded `contextual_ai` and unknown chat capabilities fail closed; account catalog guards exclude unrelated legacy entitlements; account export now includes only owned Pass and aggregate usage metadata without manuscript text; the authenticated Free account and provider path requires staging auth/provider |
