...
| G3 | Registered Free user sees roadmap, free capabilities and paid gates | BLOCKED_EXTERNAL | Local capability matrix proves bounded `contextual_ai` and unknown chat capabilities fail closed; shared entitlement and account catalog guards exclude unrelated legacy entitlements; account export includes only owned Pass and aggregate usage metadata without manuscript text; the authenticated Free account and provider path requires staging auth/provider |
...
| G7 | Lekta check, sanitized handoff, remediation and fresh re-check | BLOCKED_EXTERNAL | Browser E2E exists; material deletion now fails closed without the canonical tombstone contract, while a complete run still requires configured Lekta preview and DOCX fixture execution |
...
| G9 | Network, session, provider, balance, checkout and malformed-state failures recover honestly | BLOCKED_EXTERNAL | Local route/unit failure tests, Playwright malformed-state recovery, active-run resume, stale-run marker recovery, terminal failed-run recovery, local proposal-decision recovery, worker project-lock fail-closed coverage and lockfile-aligned/configured browser workflow checks pass; missing external Lekta preview is skipped on ordinary PRs but required by manual release, while authenticated provider/session/payment recovery still requires staging |
