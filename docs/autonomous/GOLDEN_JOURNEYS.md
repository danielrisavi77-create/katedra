...
| G9 | Network, session, provider, balance, checkout and malformed-state failures recover honestly | BLOCKED_EXTERNAL | Local route/unit failure tests, Playwright malformed-state recovery, active-run resume, stale-run marker recovery, and terminal failed-run recovery coverage pass; authenticated provider, session and payment failure recovery requires staging |
